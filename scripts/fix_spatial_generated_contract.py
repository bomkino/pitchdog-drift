...
def main() -> None:
    (ROOT / "src/engine/evaluate.ts").write_text(EVALUATE, encoding="utf-8")

    validation_path = ROOT / "src/lib/settingsValidation.ts"
    validation = validation_path.read_text(encoding="utf-8")
    if "type Flow," not in validation:
        anchor = "type DynamicsMode,"
        if anchor not in validation:
            raise RuntimeError("Could not add Flow to validation model import")
        validation = validation.replace(anchor, "type Flow,\n  type DynamicsMode,", 1)
