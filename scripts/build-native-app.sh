...
if [[ "$UI_MODE" == canonical ]]; then
    UI_PACKAGE="$ROOT/macos/NativeCore/.build/checkouts/pitchdog-studio-ui"
    UI_REVISION="$(python3 - <<'PY'
import json
from pathlib import Path
pins=json.loads(Path('macos/NativeCore/Package.resolved').read_text())['pins']
pin=next(p for p in pins if p['identity']=='pitchdog-studio-ui')
assert pin['location']=='https://github.com/bomkino/pitchdog-studio-ui.git'
print(pin['state']['revision'])
PY
)"
    [[ "$(git -C "$UI_PACKAGE" rev-parse HEAD)" == "$UI_REVISION" && -z "$(git -C "$UI_PACKAGE" status --porcelain)" ]] || {
        echo 'The resolved canonical studio UI checkout differs from its recorded source.' >&2; exit 1;
    }
fi
...
mkdir -p "$APP/Contents/Resources/ThirdPartyNotices/PitchdogStudioUI"
cp "$UI_ROOT/LICENSE" "$APP/Contents/Resources/ThirdPartyNotices/PitchdogStudioUI/LICENSE"
cp "$UI_PACKAGE/README.md" "$APP/Contents/Resources/ThirdPartyNotices/PitchdogStudioUI/README.md"
printf '%s\n' "$UI_REVISION" > "$APP/Contents/Resources/ThirdPartyNotices/PitchdogStudioUI/SOURCE_SHA.txt"
...
APP="$APP" VERSION="$VERSION" BUILD="$BUILD" SOURCE="$SOURCE" SIGNING="$SIGNING" UI_REVISION="$UI_REVISION" UI_MODE="$UI_MODE" python3 - <<'PY'
...
(app/'Contents/Resources/BuildIdentity.json').write_text(__import__('json').dumps({'source':source,'version':version,'build':build,'architecture':'arm64','minimumMacOS':'13.3','runtime':'native','signed':'ad-hoc' if os.environ['SIGNING']=='-' else 'Developer ID','notarization':'verify distributed app staple','studioUI':{'mode':os.environ['UI_MODE'],'revision':os.environ['UI_REVISION'],'repository':'https://github.com/bomkino/pitchdog-studio-ui.git' if os.environ['UI_MODE']=='canonical' else None}},indent=2)+'\n')
PY
