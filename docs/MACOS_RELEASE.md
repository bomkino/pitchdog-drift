...
6. For the explicitly authorized **0.5.1 hotfix only**, squash-merge the accepted PR with `[release-0.5.1]` in the main commit title. `source-release.yml` considers only a successful trusted push-to-main CI run carrying that marker, requires version `0.5.1`, and publishes its exact frozen installer as stable/Latest. Check for an existing or in-progress publisher before any fallback dispatch. If needed, dispatch the same workflow from main with `source_commit` equal to the exact current main SHA, `build_run` equal to its successful push-to-main CI run ID, and `prerelease=false`. Ordinary releases still require explicit dispatch (default `prerelease=true`); this one-version exception does not approve future stable releases or certify unavailable human/hardware checks. The workflow verifies authority and downloads `drift-native-installer-SHA`. The publisher verifies files, main, tree and tag, resolves drafts by release ID, resumes matching partial uploads, and refuses differing or incomplete public assets. It never replaces published bytes or moves a tag. Keep main fixed throughout publication and public readback.
7. Read the public release back, independently redownload all four assets, verify the receipt/checksum and mount the public DMG. Confirm the release channel, tag SHA and build identity. For a stable release confirm `latest`; for a prerelease confirm latest stable is unchanged. A release is complete only after destination proof.
