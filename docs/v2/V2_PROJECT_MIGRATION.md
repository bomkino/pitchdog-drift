...
Opening an older `.pitched` file may migrate the verified copy held by Drift. It does not rewrite the source file. In a browser, Save initiates a download and cannot read the browser-selected destination back. In the native app, Save resolves only after SHA-256 and byte-length agreement across the generated archive, staged broker file, and committed destination readback. Save As cancellation preserves the previous binding; external modification marks a conflict and keeps the in-app revision dirty. The packaged exact-destination exercise remains a release-live gate even though the transaction contract is source-tested.
