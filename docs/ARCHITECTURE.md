...
MP4 contains at most one AAC track. When presenter speech and sound design coexist, Drift renders both into one exact-duration 48 kHz stereo PCM master before AAC encoding. Presenter timestamps and intentional packet gaps remain truthful, mono narration cannot collapse lateral foley, and the under-voice control applies only to the tactile bed. Completed output passes the existing codec, duration, sync, channel, sample-rate, and decoded-probe readback gates.

All cue bytes are local, pinned, and hash-ledgered. Vite emits the committed WAV recordings as hashed same-origin files; the preview cache fetches them only after trusted user intent, and export can load them without any third-party origin. A separate non-destructive treatment ledger supplies trims and gain correction to both preview and export, while production gates remeasure onset, active-event energy, peak, and source integrity. See [Sonic design](SONIC_DESIGN.md).
