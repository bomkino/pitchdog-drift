...
- The normal AppKit launch path—not a test-only delegate—is invoked through LaunchServices (`open -W -n`), retains its application delegate, and presents a visible regular-app main window within 15 seconds. This gate runs before the packaged-WebView matrix so a healthy test harness cannot hide a windowless Finder or Dock launch.
