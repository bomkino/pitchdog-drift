...
The construction workflows do not merge, tag, create a GitHub Release, publish a binary, or mark a draft ready. The `v0.2.0` source tree can prepare release notes without making its tag public. The historical `v0.1.0` Release and its ad-hoc, unnotarized arm64 DMG do not satisfy the binary gates. Release documentation describes the additional Developer ID, notarisation, Gatekeeper, checksum, hardware, and human-acceptance gates.
