...
export function getSonicDensityStep(density: number): number {
  if (!Number.isFinite(density) || density <= 0.01) return Number.POSITIVE_INFINITY;
...
/**
 * Builds the exact editorial sound events for an exported timeline. It shares
 * the carousel's distance evaluator rather than guessing from frame rate, so a
 * slide crossing sounds the same at 24, 25, or 30 fps.
 */
export function buildSonicTimeline(
  ...
  const travel = Math.abs(distanceAtTime(settings, duration, slotCount, geometry.stride, true));
  ...
  for (let crossing = 1; crossing <= crossingCount; crossing += 1) {
    ...
    const lead = clamp(0.08 / Math.max(slidesPerSecond, 0.18), 0.024, 0.105);
    const time = crossingTime - lead;
    // A cue exactly on the loop seam is heard twice when social players repeat.
    if (time < 0 || time >= duration - 0.045) continue;

    const unit = hashUnit(baseSeed + crossing * 977);
    const signedVariation = (unit * 2 - 1) * settings.sound.variation;
    events.push({
      cue: "passage",
      time,
      gain: clamp(0.42 + intensity * 0.43 + signedVariation * 0.05, 0.26, 0.92),
      playbackRate: clamp(baseRate * (1 + signedVariation * 0.09), 0.78, 1.2),
  ...
  if (!settings.motion.seamless && events.length > 0) {
    ...
    if (settleTime - last.time > 0.22) {
      const unit = hashUnit(baseSeed ^ 0x51f15e);
      const signedVariation = (unit * 2 - 1) * settings.sound.variation;
      events.push({
        cue: "settle",
        time: settleTime,
        gain: clamp(0.28 + intensity * 0.16, 0.22, 0.52),
        playbackRate: clamp(baseRate * (1 + signedVariation * 0.05), 0.82, 1.15),
        pan: 0,
      });
    }
  }
