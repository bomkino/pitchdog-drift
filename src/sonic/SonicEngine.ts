...
import { getSonicAssetBytes, SONIC_CUES, type SonicCue } from "./catalog";
...
export class SonicEngine {
  ...
  private readonly decoded = new Map<SonicPalette, Map<SonicCue, AudioBuffer>>();
  private readonly loads = new Map<SonicPalette, Promise<Map<SonicCue, AudioBuffer>>>();
  ...
  setSettings(settings: SonicSettings): void {
    ...
    if (paletteChanged && this.context && !this.unavailable) {
  ...
  play(cue: SonicCue, gesture: SonicGesture = {}): boolean {
    ...
    if (cue === "passage") {
      ...
      if (!Number.isFinite(densityStep) || (this.motionSequence - 1) % densityStep !== 0) return false;
    }
    ...
    this.sequence += 1;
    const variation = (hashUnit(this.sequence + cue.length * 17) * 2 - 1) * this.settings.variation;
    const intensity = clamp(gesture.intensity ?? 0.7, 0.08, 1);
    const familyGain = MOTION_CUES.has(cue) ? this.settings.motionGain : this.settings.interfaceGain;
    const source = context.createBufferSource();
    const gain = context.createGain();
    const panner = context.createStereoPanner();
    source.buffer = buffer;
    source.playbackRate.value = clamp(1 + variation * 0.085, 0.8, 1.2);
    ...
    gain.gain.linearRampToValueAtTime(voiceGain, start + Math.min(0.012, buffer.duration * 0.2));
    gain.gain.setValueAtTime(voiceGain, Math.max(start + 0.012, end - 0.024));
  ...
  async suspendForVisibility(hidden: boolean): Promise<void> {
    ...
    if (hidden && context.state === "running") await context.suspend().catch(() => undefined);
    else if (!hidden && context.state === "suspended" && this.settings.previewEnabled) {
  ...
  private updateMasterGain(): void {
    ...
    const target = this.settings.previewEnabled && !this.suppressed ? this.settings.masterGain : 0;
  ...
  private fail(error: unknown): void {
    this.unavailable = true;
    this.onError?.(error instanceof Error ? error.message : "Tactile sound could not start in this browser.");
    this.publishState();
  }
