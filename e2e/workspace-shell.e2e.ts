...
async function openAdvanced(page: Page): Promise<void> {
  const trigger = page.getByTestId("workspace-scroll").locator(".workspace-advanced-trigger");
...
test("workspace switches expose one persistent inspector pane and keep stage geometry within one pixel", async ({ page }) => {
  ...
  for (const [label, id, primaryGroup] of expectations) {
    ...
    await expect(page.locator("[data-workspace-content]")).toHaveCount(4);
    await expect(page.locator('.workspace-scroll[data-active="true"] [data-workspace-content]')).toHaveCount(1);
    await expect(page.locator(`[data-workspace-pane="${id}"] [data-workspace-content="${id}"]`)).toBeVisible();
    await expect(page.getByTestId("workspace-scroll").locator(".inspector-group").filter({ hasText: primaryGroup }).first()).toBeVisible();
    if (id === "look") {
  ...
  const activeInspector = page.getByTestId("workspace-scroll");
  await expect(activeInspector.getByRole("combobox", { name: "Background", exact: true })).toHaveCount(0);
  await expect(activeInspector.getByRole("group", { name: "Flow axis" })).toHaveCount(0);
  await expect(activeInspector.getByLabel("Slide ratio", { exact: true })).toHaveCount(0);
});
...
test("selection, reorder, and pinning preserve workspace, stage, disclosure, focus, and scroll", async ({ page }) => {
  ...
  const openBefore = await page.locator(".workspace-scroll[data-active='true'] [data-disclosure][data-expanded='true'] > button, .workspace-scroll[data-active='true'] details[open] > summary").allTextContents();
  ...
  expect(Math.abs(await readWorkspaceScroll(page) - scrollBefore)).toBeLessThanOrEqual(1);
  expectInvariant(await stageBox(page), stageBefore);
  expect(await page.locator(".workspace-scroll[data-active='true'] [data-disclosure][data-expanded='true'] > button, .workspace-scroll[data-active='true'] details[open] > summary").allTextContents()).toEqual(openBefore);
});
...
test("outcome cards apply and undo complete motion recipes without changing Look or stage geometry", async ({ page }) => {
  await waitForStudio(page);
  await switchWorkspace(page, "LOOK");
  const backgroundBefore = await page.locator(".current-background strong").textContent();
  const backgroundFamilyBefore = await page.locator(".current-background-stage .background-preview").getAttribute("data-family");
  const stageBefore = await stageBox(page);
  ...
  await page.getByRole("button", { name: /Apply clean carousel/i }).click();
  await expect(page.getByRole("button", { name: /Clean carousel active/i })).toBeDisabled();

  await switchWorkspace(page, "LOOK");
  await expect(page.locator(".source-fidelity-strip strong")).toHaveText("Literal source pixels");
  await expect(page.locator(".current-background strong")).toHaveText(backgroundBefore!);
  await expect(page.locator(".current-background-stage .background-preview")).toHaveAttribute("data-family", backgroundFamilyBefore!);
  expectInvariant(await stageBox(page), stageBefore);
});

test("content-paced Smooth and Casino preserve readable timing when the moving count changes", async ({ page }) => {
  ...
  await casino.click();
  await expect(timing.locator("strong")).toHaveText("11.95 s master");
  await page.getByRole("button", { name: "Keep Drift study 01.png still" }).click();
  await expect(timing).toContainText("7 MOVING");
...
test("supplementary workspace help opens for keyboard, escapes cleanly, and stays inside the viewport", async ({ page }) => {
  ...
  await expect(tooltip).toHaveText("Choose a direction, axis, path, pace, and complete-deck timing.");
  const describedBy = await motionTab.getAttribute("aria-describedby");
  ...
  await expect(motionTab).toBeFocused();
  await expect(motionTab).not.toHaveAttribute("aria-describedby", /drift-tooltip/u);
