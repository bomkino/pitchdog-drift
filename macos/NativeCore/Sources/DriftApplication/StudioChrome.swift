...
struct DriftChrome: ViewModifier {
    ...
    func body(content: Content) -> some View {
        #if canImport(PitchdogStudioUI)
        content.studioSurface(.window).studioTheme(theme).buttonStyle(StudioButtonStyle()).textFieldStyle(DriftFieldStyle()).studioType(.bodyCompact).studioTypography(DriftType.typography)
