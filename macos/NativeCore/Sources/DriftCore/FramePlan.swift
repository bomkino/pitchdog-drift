...
public struct FramePlan:Sendable {
    ...
    private let slideTrack:SlideTrack
    public init(project:DriftProject)throws {
        ...
        slideTrack=SlideTrack(project:project,slides:moving)
        var cues:[CueTiming]=[]
    ...
    public func movingPoses(baseSeconds:Double,interaction:Double=0)->[CardPose] {
        ...
        let width=w*bounded(c.card.scale,0.1,1.6),axis=vertical ? h:w,cross=vertical ? w:h
        let extent=slideTrack.maximumExtent,radius=axis/2+extent
        let copies=slideTrack.copies(covering:axis),count=moving.count*copies,length=Double(copies)*slideTrack.length
        let travelled=slideTrack.position(at:distance,copies:copies)
        var cards:[CardPose]=[]
        for slot in 0..<count {
            let i=slot%moving.count,slide=moving[i]
            let center=Double(slot/moving.count)*slideTrack.length+slideTrack.centers[i]
            let primary=positiveModulo(center-travelled+length/2,length)-length/2
            if abs(primary)>radius+extent*1.25{continue}
            ...
            let t=layer(sample,index:i,travel:slideTrack.footprints[i].height*0.055)
